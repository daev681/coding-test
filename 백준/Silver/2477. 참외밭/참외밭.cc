...
int main(){
    ...
    // 가로/세로 중 최대값 찾기
    for (int i = 0; i < 6; i++) {
        if (direction[i] == 1 || direction[i] == 2) { // 동 or 서 → 가로
      ...
      // 작은 사각형 찾기
    // (큰 변의 양 옆에 있는 변끼리 곱함)
    int smallWidth = abs(length[(maxHeightIdx + 5) % 6] - length[(maxHeightIdx + 1) % 6]);
    int smallHeight = abs(length[(maxWidthIdx + 5) % 6] - length[(maxWidthIdx + 1) % 6]);
